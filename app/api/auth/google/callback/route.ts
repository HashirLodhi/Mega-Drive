...
export async function GET(request: NextRequest) {
  ...
  try { const code = request.nextUrl.searchParams.get("code"); const state = request.nextUrl.searchParams.get("state"); const expected = request.cookies.get("megadrive_oauth_state")?.value; const workspaceId = workspaceFromRequest(request); if (!code || !state || !expected || state !== expected || !workspaceId) throw new Error("OAuth validation failed. Please connect again."); const token = await exchangeCode(code, redirectUri); const profile = await readProfile(token); await saveAccount({ workspaceId, id: profile.sub, email: profile.email, name: profile.name || profile.email, picture: profile.picture, token, connectedAt: new Date().toISOString() }); const response = NextResponse.redirect(new URL("/?connected=1", origin)); response.cookies.delete("megadrive_oauth_state"); return response; }
