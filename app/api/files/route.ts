...
export async function GET(request: NextRequest) { try { const accountId = request.nextUrl.searchParams.get("accountId"); const workspaceId=workspaceFromRequest(request);if(!workspaceId)return NextResponse.json({error:"Workspace session is required"},{status:401}); if (!accountId) return NextResponse.json({ error: "Select an account" }, { status: 400 }); const account = await getAccount(workspaceId,accountId); if (!account) return NextResponse.json({ error: "Account not found" }, { status: 404 }); return NextResponse.json(await listDriveFiles(account, { trashed: request.nextUrl.searchParams.get("trashed") === "true", query: request.nextUrl.searchParams.get("q") ?? undefined, pageToken: request.nextUrl.searchParams.get("pageToken") })); } catch (error) { return NextResponse.json({ error: (error as Error).message }, { status: 502 }); } }
